...
class ContractAdaptor(STS):
                ...
                if length == 1:
                    ...
                    for el in vect.getElements().values():
                        if el.isEmission():
                            ...
                            LOG.warn("There was a vector element which was \
                               not IN nor OUT")
                else:
                    LOG.warn("There was a vector with more than one element")
                    continue
                label = None
                #LOG.debug("Vector: %s; Vector element: %r; Op name: %s" % \
                #       (vect, ele, ele.getName()))
                if ele.isEmission():
    ...
    def _getNewState(self, contract_state, queue):
        ...
        if stateName not in self.getStates():
            ...
            #LOG.debug("Number of generated states: {0}".format(\
            #       len(self.getStates())))
        return stateName
...
class DynamicContractAdaptor(ContractAdaptor):
    ...
    # @TODO: I'll remove this class sooner than later
...
class DetContractAdaptor(STS):
    ...
    def outgoingTransitions(self, state):
        ...
        if not toReturn:
            ...
            for label in labels:
                ...
                toReturn.add(self._createTransition(state, label_instance,
                             dstate))

            if LOG_DET.isEnabledFor(logging.DEBUG):
                LOG_DET.debug(
                    "There are {0!s} inner outgoing transitions".format(
                    str(self.__getStateIterable(state))))
                LOG_DET.debug("Created {0} new transitions.".format(
                    str(len(labels))))
    ...
    def _getNewState(self, iterable_states):
        ...
        @param iterable_states: the destination states under the same label
        """
        cstate = self.__createCompositeState(iterable_states)
        if cstate not in self.getStates():
            isFinal = False
            #LOG_DET.debug("States: {0}; Inner finals: {1}".format(
            #    str(iterable_states), str(self._inner.getFinals())))
...
class DynamicDetContractAdaptor(DetContractAdaptor):
    ...
    def __init__(self, contract):
        super(DynamicDetContractAdaptor, self).__init__(contract)
...
class LearningAdaptor(DetContractAdaptor):
    ...
    possible mistakes, therefore candidates to be inhibited. The reason for
    this is that these transitions were not actually synchronised and
    therefore, not properly learned.
    """
    ...
    def outgoingTransitions(self, state):
        ...
        transitions = [t for t in DetContractAdaptor.outgoingTransitions(
                self, state) if
    ...
    def inhibit(self, transition):
        ...
        if transition in self.inhibited:
            LOG.debug("Transition already inhibited! {0}".format(
                hash(transition)))
        else:
    ...
    class SubscriberInhibiter(object):
        ...
        def __init__(self, adaptor, position):
            ...
            @param adaptor: adaptor whose transitions are dynamically inhibited
        ...
        def notify(self, reason, details):
            ...
            LOG.debug("Notified about an UNFINISHED_TRACE: {}".format(\
                    details))
            #LOG.warn("The dreadful trace is : {}".format(
            #         trace_to_string(details, 0)))
            if not details:
                LOG.warn("Something strange is happening inhibiting on "+ \
                         "real time")
            adaptor_current_state = details[-1][0][self.position]
            # Inhibit only if the adaptor is not a contract's accepted state
            if adaptor_current_state not in self.adaptor.getFinals():
                to_inhibit = self.adaptor.incomingTransitions(
                        adaptor_current_state)
                if len(to_inhibit) > 1:
    ...
    def removeDescendants(self, state):
        """Remove every transition and state from (including) the given state
        ...
        if state in self.getStates():
            ...
            # This removes incoming and outgoing transitions from state
            self.removeStateOnly(state)


    def getTransitions(self):
        # @TODO: Maybe this does not work because, 
        #        in LTS, getTransitions return a list.
...
class AdaptiveAdaptor(DynamicThresholdAdaptor):
    ...
    def alterThreshold(self, reason):
        ...
        if reason is SynchroniserFeedback.SUCCESSFUL_TRACE or \
            ...
            # @TODO: This should use getExploredTransitions(), it's inefficient
...
def test():
    ...
    suite = unittest.TestLoader().loadTestsFromName("suite", \
            itacalib.tests.adaptor.all_tests)
...
""")
    ...
    group_adaptor.add_argument('--sthr', metavar="THR", type=int, \
            help="the static threshold beyond the adaptor forgets, "+
                 "default = infinity")
    ...
    if args.dter:
        try:
            dter = eval(args.dter)
            # @TODO: @TOREFACTOR: Don't use asserts for this
    ...
    if args.prefix:
        ...
        # Not elegant but...
        final_adaptor.getExploredTransitions = final_adaptor.getTransitions
